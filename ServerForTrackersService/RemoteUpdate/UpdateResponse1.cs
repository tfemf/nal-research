...
// Type: Nal.RemoteUpdate.UpdateResponse1
...
{
  public class UpdateResponse1 : UpdateResponse
  {
    public UpdateResponseStatus Status { get; set; }

    public UpdateRequest1 Request { get; set; }
    ...
    public override XElement ToXElement()
    {
      ...
      return new XElement((XName) "updateResponse1", new object[3]
      {
    ...
    public static bool Parse(IList<byte> data, out UpdateResponse1 response)
    {
      response = (UpdateResponse1) null;
      if ((data.Count == 29 || data.Count == 32) && data[0] == (byte) 118 && data[1] == (byte) 1)
      {
        byte num1 = 0;
        for (int index = 0; index < 28; ++index)
          num1 ^= data[index];
        if ((int) num1 == (int) data[28])
        {
          try
          {
            ...
            byteList.AddRange(data.Skip<byte>(25).Take<byte>(3));
            byte num2 = 0;
            for (int index = 0; index < byteList.Count; ++index)
              num2 ^= byteList[index];
            byteList.Add(num2);
            UpdateRequest1 update;
            if (UpdateRequest1.Parse(byteList.ToArray(), out update))
            {
